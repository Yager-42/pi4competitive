...
| Doc | Role |
|-----|------|
| [`docs/contracts/ARCHITECTURE_CONTRACT.md`](docs/contracts/ARCHITECTURE_CONTRACT.md) | **v0.3.9** — process, paths, imports, stack; P3 local subset (ADR 0006); **P3.1/P3.2** (ADR 0008/0009); **P3.3 native AgentTool sandbox (ADR 0012；ADR 0011 Docker-specific historical)**; P4 references/workflow (ADR 0007/0010) |
...
| [`docs/features/agent_tool_native_sandbox_v1.md`](docs/features/agent_tool_native_sandbox_v1.md) | AgentTool native sandbox **frozen v0.2.1**（P3.3；G1–G10；G0 complete；Python port `pi-sandbox@0.4.2` + SRT `0.0.67` + `pi-auto-review@0.3.2`；Linux/macOS；native-only） |
| [`docs/features/agent_tool_sandbox_v1.md`](docs/features/agent_tool_sandbox_v1.md) | Historical Docker sandbox feature **v0.1.33 superseded**；provider-neutral executor/RPC/scope outputs inherited by ADR 0012 |
...
| [`docs/plans/P3_3_agent_tool_native_sandbox.md`](docs/plans/P3_3_agent_tool_native_sandbox.md) | P3.3 native AgentTool sandbox implementation plan (**v0.1.1 active**；G0 complete，A–F/V1–V4 not started；binding [`G0 map`](docs/plans/P3_3_native_sandbox_G0_map.md)） |
...
| [`docs/contracts/adr/0012-native-agent-tool-sandbox-runtime.md`](docs/contracts/adr/0012-native-agent-tool-sandbox-runtime.md) | ADR: supersede Docker-specific P3.3 decisions；native-only Linux/macOS sandbox + Python approval/SRT port |
...
2. Implementation order is **serial**: P1 `packages/ai` → P2 `packages/agent` → P3 local loader → **P3.1 engine extensions** → **P3.2 capability enablement** → **P3.3 AgentTool sandbox** → continue P4 `competitive_app`.
...
5. P4 existing work remains, but do **not** expand AgentTool-dependent P4 business features until the P3.3 plan's exit gates pass.
...
1. Read this file + architecture contract v0.3.9 + the active frozen native feature/plan.
