...
## Current status (as of contract v0.3.13 / roadmap v0.1.53)
...
- **P3.2** Pi extension capability enablement (generic bridge + upstream cache parity) — **done** (ADR 0009)
- **P3.3** native AgentTool sandbox (`adapter/out/sandbox/native/`) — **done** (ADR 0013/0014/0016; Linux real-gate suite V2 is optional per ADR 0014 but **must pass before any Linux production deploy**)
- **P4** `competitive_app` — **in progress**: DDD + FastAPI + 30 HTTP routes; research workflow v2 (SearchOS coverage engine, ADR 0010) implemented; LLM fallback + observability implemented (ADR 0015); frontend implemented. Features `competitive-app-http-v1` v0.3.5, `research-workflow-v1` v0.2.8, `competitive-app-frontend-v1` v0.3.1, `llm-fallback-observability-v1` v0.2.1.
- **Business capability v1 (search)** — **done**: three search capability packages + five AgentTools, wired into the workflow through the native sandbox.

P1–P3.3 are complete. P4 is the only active phase; it iterates by feature (each with its own frozen boundary contract under `docs/features/`), not by a single exit gate.
...
| [`docs/contracts/ARCHITECTURE_CONTRACT.md`](docs/contracts/ARCHITECTURE_CONTRACT.md) | **v0.3.13 frozen baseline** — binding decisions, quality gates |
| [`docs/ROADMAP.md`](docs/ROADMAP.md) | Serial phases P1→P2→P3→P3.1→P3.2→P3.3→P4, status board, exit gates |
| [`docs/features/`](docs/features/) | Per-feature boundary contracts (frozen). Notably `agent_tool_native_sandbox_v1.md` (v0.2.4), `competitive_app_http_v1.md` (v0.3.5), `research_workflow_v1.md` (v0.2.8), `agent_engine_extensions_v1.md` (v0.3.0), `search_capability_packages_v1.md` (v0.1.12) |
| [`docs/plans/`](docs/plans/) | Per-phase implementation plans + module maps (P1–P4, incl. P3.1/P3.2/P3.3) |
| `docs/contracts/adr/*` | ADRs 0001–0016 |
...
`uv` workspace (members: `packages/ai`, `packages/agent`, both editable-installed). pytest uses `--import-mode=importlib`, `asyncio_mode = "auto"`, markers `live` (real provider calls), `slow`, `native_sandbox`, `linux`, `macos`.
...
uv run pytest tests/competitive_app -m "not live" -q # P4 suite
...
### Runtime requires POSIX (WSL on Windows)

The AgentTool sandbox is **mandatory and has no fallback** (ADR 0013), and it is POSIX-only: it uses `os.O_NOFOLLOW` and `dir_fd=` — neither exists on Windows (`hasattr(os, "O_NOFOLLOW")` → `False`, `"mkdir" in os.supports_dir_fd` → `False`). The backend cannot start on Windows and pytest cannot even collect the sandbox suites there. On a Windows checkout, run everything through WSL2 with a separate venv so the Windows `.venv` is not clobbered:

```bash
wsl -e bash -lc "cd /mnt/e/project/pi4competitive && UV_PROJECT_ENVIRONMENT=.venv-linux uv run pytest -m 'not live' -q"
```

Real Linux enforcement (bubblewrap/seccomp) additionally needs `bwrap` present; without it the SRT falls back to the faux broker and the `linux` marker suites are skipped.
...
P3.2 extension bridge    Pi extension capability enablement + cache parity       → done
P3.3 sandbox/native/     Pi executor seam + App native SRT/approval adapter      → done
P4  competitive_app      FastAPI + DDD + workflow (business features)            → in progress
```
...
**P4 layering:** DDD/hexagonal — `adapter/in_/fastapi` → `application/` (`workflow` Process Manager, `model`, `evolution`) → `domain` (pure, no IO) → `adapter/out/` (`persistence`, `sandbox`, `observability`). `wiring.py` is the composition root — it owns every policy that needs both halves (sandbox egress gate, LLM fallback chain, capability package filters). FastAPI routes must not stage orchestration; domain must not do IO.
...
**`packages/ai`** (`earendil_works.pi_ai`, v0.81.3 = upstream 0.81.1 baseline + ADR 0012 `response_format` passthrough + ADR 0015 structured error): `create_models`/`ModelsImpl` (`models.py`), `Model`/`Context`/`Tool`/`Usage`/`AssistantMessage` (`types.py`), ~37 provider factories in `providers/` (each with a `*_models.py` catalog from npm `@earendil-works/pi-ai@0.81.1`), wire-protocol builders + lazy loaders in `api/`, credential resolution/oauth in `auth/`. `faux_provider`/`faux_text`/`faux_tool_call` are offline test doubles.
...
Load + apply: `report = await load_capability_packages(); apply_capability_report(agent, report)` (filters: `enabled=[...]`/`disabled=[...]`/`strict=True`). Seven packages exist: `search_tavily`, `search_anysearch`, `search_grok` (business capability v1), `pi_auto_review` (boundary approval reviewer, behavior-equivalent port of `pi-auto-review@0.3.2`), `reasonix_prefix_cache` (P3.2 upstream cache parity), `learned_skills` (skills-only, workflow self-evolution), `echo_example` (loader smoke). **Capability packages run with full process privileges** — review before enabling; no remote download path exists.
...
10. **AgentTool sandbox is mandatory and has no fallback** (ADR 0013, D16/G8): every AgentTool executes through the native provider (Linux bubblewrap/seccomp, macOS Seatbelt). There is no host/Direct/Local/Docker escape hatch — don't add one to "make it work on Windows".
11. **Sandbox egress goes through the App composition root** (ADR 0016, G14): `wiring._build_native_review_domain` is the only host-side gate, and the broker's `validate_public_hostname` double-check (pre-ask + pre-dial re-resolve, anti-DNS-rebinding) must not be bypassed. Non-public addresses, mixed resolution, and DNS failure all deny. Egress policy must not sink into `packages/ai|agent` or the domain. Default (`SANDBOX_ALLOWED_DOMAINS` empty) allows all public addresses; production should tighten it to the provider hosts.
12. **Legacy repo is reference only.** `competitive-agent/` is for P4 business/workflow/domain shape ideas — never a Pi parent, never a 1:1 copy backlog (D12 / ADR 0007).
13. **Secrets never in git.** `.env` only; `config/settings.example.yaml` carries no live keys (scanned by `test_settings_hygiene.py`).
14. **PR title prefix** with the stage, e.g. `P4: search capability wiring`. Update the plan's status board rows as you complete them.
