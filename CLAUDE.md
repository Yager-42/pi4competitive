...
A third repo, [`antins-labs/SearchOS`](https://github.com/antins-labs/SearchOS) (checked out alongside as
`SearchOS/`), is the **engine architecture reference only** for the P4 research workflow (coverage map / SOCM /
Extraction / Sensor concepts) — not a code parent, not a 1:1 copy (ADR 0010).

## Current status (as of contract v0.3.12 / roadmap v0.1.52)
...
- **P3.2** Pi extension capability enablement — **done** (ADR 0009)
- **P3.3** AgentTool native sandbox — **done** (ADR 0013/0014: Linux bubblewrap/seccomp + macOS Seatbelt; Linux real-enforcement gate optional, macOS gate required)
- **P4** `competitive_app` — **in progress**: DDD + FastAPI + research workflow + React frontend all landed. Research workflow **frozen** (`research-workflow-v1` v0.2.8, three-stage plan/search/write + SearchOS coverage engine); HTTP boundary **frozen** (`competitive-app-http-v1` v0.3.5, 29 routes). See [`docs/features/research_workflow_v1.md`](docs/features/research_workflow_v1.md) and [`docs/features/competitive_app_http_v1.md`](docs/features/competitive_app_http_v1.md).
- **Business capability v1 (search)** — three search capability packages + AgentTools implemented and feature-frozen; wired into the research workflow (per-task `search_overrides` landed at http v0.3.5).
...
| [`docs/contracts/ARCHITECTURE_CONTRACT.md`](docs/contracts/ARCHITECTURE_CONTRACT.md) | **v0.3.12 frozen baseline** — binding decisions, quality gates |
| [`docs/ROADMAP.md`](docs/ROADMAP.md) | Serial phases P1→P3→P3.1→P4, status board, exit gates |
| [`docs/features/`](docs/features/) | Per-feature boundary contracts (frozen). `search_capability_packages_v1.md` (v0.1.12), `agent_engine_extensions_v1.md` (v0.3.0), `agent_tool_native_sandbox_v1.md` (v0.2.3) |
| [`docs/plans/`](docs/plans/) | Per-phase implementation plans + module maps (P1–P4, P3.1) |
| `docs/contracts/adr/*` | ADRs 0001–0015 |
...
`uv` workspace (members: `packages/ai`, `packages/agent`, `competitive_app`). pytest uses `--import-mode=importlib`, `asyncio_mode = "auto"`; markers `live` (real provider calls), `slow`, `native_sandbox`, `linux`, `macos`.
...
uv run pytest tests/competitive_app -m "not live" -q # P4 app suite
...
uv run python scripts/serve_app.py                   # dev backend :8010 (uvicorn reload); --port override
bash scripts/fetch_upstream.sh                       # sparse-checkout upstream packages → vendor/

# Frontend (React 19 + Vite + TS, node 22; lives in frontend/):
cd frontend && npm install && npm run dev            # dev server :3400, proxies /api → :8010
cd frontend && npm run build                         # tsc -b && vite build
```

`data/` (incl. `data/sessions/` JSONL) and `.env` are gitignored. Live tests are env-gated and **not** exit-blocking — faux + JSONL smoke is the gate. Full-stack dev = `serve_app.py` backend (:8010) + `frontend` Vite (:3400).
...
P3.2  pi extension         capability enablement (ADR 0009)                         → done
P3.3  AgentTool sandbox    native OS sandbox runtime (ADR 0013/0014)               → done
P4    competitive_app      FastAPI + DDD + research workflow + frontend             → in progress
```
...
**P4 layering (built):** DDD/hexagonal — `adapter/in/fastapi` (29 HTTP routes) → `application/workflow` (Process Manager / three-stage plan→search→write runner) → `domain` (pure, no IO; `domain/socm` = SearchOS coverage map) → `adapter/out/persistence` (SQLite projection) + `adapter/out/sandbox` (P3.3 native runtime). FastAPI routes must not stage orchestration; domain must not do IO. `frontend/` (React + Vite + TS) is a view over these `/api` routes — never a second source of truth.
...
Load + apply: `report = await load_capability_packages(); apply_capability_report(agent, report)` (filters: `enabled=[...]`/`disabled=[...]`/`strict=True`). Seven packages exist: `echo_example`, `search_tavily`, `search_anysearch`, `search_grok`, `learned_skills`, `pi_auto_review`, `reasonix_prefix_cache`. **Capability packages run with full process privileges** — review before enabling; no remote download path exists.
