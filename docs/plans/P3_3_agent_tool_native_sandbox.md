...
| **plan_version** | `0.1.7` |
| **status** | **complete — A–F + V1 + V3（macOS real gate）+ V4（audits/baseline/closeout）done；V2 optional per ADR 0014（未运行，Linux deploy 前必过）** |
| **created** | 2026-08-02 |
| **updated** | 2026-08-03 |
| **roadmap** | [`docs/ROADMAP.md`](../ROADMAP.md) stage P3.3 |
| **contract** | [`ARCHITECTURE_CONTRACT.md`](../contracts/ARCHITECTURE_CONTRACT.md) v0.3.11 |
| **ADR** | [`0013-native-agent-tool-sandbox-runtime.md`](../contracts/adr/0013-native-agent-tool-sandbox-runtime.md) accepted |
| **feature** | [`agent_tool_native_sandbox_v1.md`](../features/agent_tool_native_sandbox_v1.md) frozen v0.2.3 |
...
| **G0 map** | [`P3_3_native_sandbox_G0_map.md`](P3_3_native_sandbox_G0_map.md) v0.1.1 frozen/complete |
...
| Phase | Work | Status |
|-------|------|--------|
| G0 | frozen source/license/baseline capture | **done — G0 map v0.1.1** |
...
| V2 | Linux amd64 real enforcement/e2e | **optional（ADR 0014）**— Linux 主机/CI 可用时运行同一 S1–S9 套件；不阻塞 closeout；任何 Linux production 部署声明前必过 |
...
> **ADR 0014（contract v0.3.11）**：§5.2 Linux real gate 为**可选项** —
...
| Metric | Native（measured） | Frozen Docker baseline（ADR 0013） |
|--------|--------------------|-----------------------------------|
...
| PR4 | E wiring/readiness + real gates | macOS real gate e2e green（required）；Linux real gate optional per ADR 0014（Linux 主机可用时运行） |
...
- [x] Linux SRT path real enforcement gate is **optional** (ADR 0014): runs
...
| Version | Date | Change |
|---------|------|--------|
| `0.1.0` | 2026-08-02 | Initial active plan from ADR 0013 / native feature v0.2.0; preserves current executor/RPC/scope, ports three frozen upstreams, replaces and removes Docker, and defines Linux/macOS real gates |
...
| `0.1.4` | 2026-08-03 | **ADR 0014**：V2 Linux real gate 改为可选项（不阻塞 closeout；Linux production 部署前必过；apply-seccomp 供应链契约不变）；V3 macOS real gate 正式必过（进行中）；roadmap/feature/contract 同步 v0.3.11 |
...
| `0.1.7` | 2026-08-03 | **合并 main 重编号**：P3.3 ADR 0012/0013 与 response_format ADR 0012 撞号 → native=ADR 0013、Linux 可选=ADR 0014；contract v0.3.11；feature 0.2.3；行为不变 |
| `0.1.6` | 2026-08-03 | **V4 closeout done**：resource baseline vs ADR 0013 Docker（cold 0.109 s vs 1.386 s；steady P50 108.9 ms/P95 110.4 ms；10-way parallel 0.85 s、in-flight 10 brokers 361 MiB；residual 0；disk 2.0 MB vs image 12.1 GB）；license audit — 补回 pi-sandbox Apache 与 pi-auto-review MIT 文本（SHA 对齐 G0 pin，O22 全量 pin 断言）；header audit 全绿；CodeGraph sync；§7 全勾选；offline 727 passed |
