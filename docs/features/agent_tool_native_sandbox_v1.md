...
| **plan** | [`P3_3_agent_tool_native_sandbox.md`](../plans/P3_3_agent_tool_native_sandbox.md) v0.1.7 complete；[`G0 map`](../plans/P3_3_native_sandbox_G0_map.md) v0.1.1 complete |
...
| G4 | 平台承诺 | Linux 与 macOS 都 production-capable；macOS real gate 必过；Linux real gate 可选（ADR 0014）— Linux 主机可用时运行，任何 Linux production 部署声明前必须通过 | RESOLVED |
...
- **macOS（arm64）real enforcement 是正式必过 gate**；**Linux real enforcement 是可选项**（ADR 0014）：Linux amd64 主机/CI 可用时运行同一 S1–S9 套件，结果作为 Linux production 部署前提；不阻塞 P3.3 closeout；
...
- **Linux（amd64）可选**（ADR 0014）：同一 S1–S9 套件在 Linux 主机运行时 green；Linux production 部署前必须执行。

### 10.3 Performance/resource evidence

必须与 ADR 0013 记录的 Docker 基线对比（优先同一台 arm64 macOS host；Linux amd64 host 可用时补充对比）：
...
2. Linux 与 macOS 使用不同的内核隔离原语；Seatbelt 没有 Linux PID/user/mount namespace，但两者都必须满足本 feature 锁定的对外行为与 fail-closed 契约。**ADR 0014 后：Linux real enforcement 未执行前，Linux production 真实行为（bubblewrap/seccomp 生效）为未验证状态**；macOS 真实行为已由必过 gate 验证。
...
| G4 | production 平台承诺 | Linux 与 macOS 都 production-capable；macOS real gate 必过；Linux real gate 可选（ADR 0014），Linux production 部署声明前必须通过 | RESOLVED |
...
G1–G10、ADR 0013/0014、architecture contract v0.3.11 与 Roadmap/plan 同步已完成；G0 source/test/removal evidence 已冻结。实现只能按 plan v0.1.7 + G0 map 推进；macOS real gate（V3）、offline parity（V1）、Docker production removal 与文档/license 审计（V4）完成前不得关闭 P3.3；Linux real gate（V2）为可选项，不阻塞关闭。
