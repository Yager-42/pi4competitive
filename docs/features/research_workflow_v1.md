...
| 字段 | 值 |
|------|-----|
| **feature_contract_version** | `0.2.9` |
| **status** | **frozen** |
| **updated** | 2026-08-09 |
| **feature_id** | `research-workflow-v1` |
| **roadmap_stage** | **P4** `competitive_app` —— 三阶段研究 workflow（SearchOS coverage 引擎复现；替换 v0.1.1 六阶段） |
| **architecture_contract** | [`ARCHITECTURE_CONTRACT.md`](../contracts/ARCHITECTURE_CONTRACT.md) **v0.3.12**（§3.2 / D8 / G1 / G2 / D24 + ADR 0010-0015） |
...
|----|-----|
| 冻结版本 | `0.2.6` |
| 冻结日期 | 2026-08-05（v0.2.6 patch；v0.2.5 patch 2026-08-02；v0.2.2 frozen 2026-07-30） |
| grill | 31 决策收敛（§8 F-R1..F-R31；v0.1.1 的 F-R1..F-R24 + v0.2.0 的 F-R25..F-R31）+ v0.2.1 补丁（D-S3'/D-S6'/D-S8'/D-S2a/D-S2b，见 ADR 0010 Patch v0.2.1）+ v0.2.2 补丁（write sections + trace span，见 §9.1）+ v0.2.3 补丁（evidence 物化投影 + clarify brief 推导，见 §9.1）+ v0.2.4 补丁（response_format 强制 JSON，ADR 0012）+ v0.2.5 补丁（跨任务记忆 recall+inject 最小切片，见 §9.1）+ v0.2.6 补丁（write 逐 section 多 call，见 §9.1） |
...
| 0.2.5 | 2026-08-02 | **patch frozen（跨任务记忆 recall+inject 最小切片）**：write stage 建 prompt 时按 brief 竞品(`target.name`+`competitors`)recall evidences 表历史 finding → 渲染按竞品分组 blob(`## {brand}` / `- {attr}: {value} (src,conf,captured_at)`)+ 变化检测指令头("compare to current search; flag old→new")→ 注入 write **user prompt**(不改 locked `system_prompt`)。**复用 evidences 表当记忆库**(不新表/不 compiler/不 consolidate/不 agent recall 工具——均留后);每 (entity,attribute) 留最新一条(captured_at desc);25KB 按竞品块整块丢 + `(memory truncated)`(UTF-8 字节截断);空召回不注入(首次/新竞品零干扰);alias 仅 case-insensitive(suffix 去除需 write-time 归一化,defer)。落点:`application/workflow/memory_inject.py`(新 helper `recall_prior_findings`)+ `research_runner._build_prompt` write 分支(lazy recall);`TaskProjectionStore.query_evidences` 加 `brands: list` case-insensitive(`lower(brand) IN ...`,向后兼容 `brand` 单参)。**诚实边界**:只保证记忆到达 write prompt;"LLM 据此标 old→new 变化"是 policy-only(对齐 competitive-analysis-agent AC11),不测、不保证。契约 0.3.9 不动(复用 evidences,非架构变更,无 ADR);http v0.3.4 不动(无新路由);ROADMAP 0.1.45;offline 12 测试(helper unit 8 + store integration 4)+ live 真 run(190s,低 SEARCH_COVERAGE_THRESHOLD 让搜索快停→write 启动→断言 blob 进 write user message) |
| 0.2.6 | 2026-08-05 | **patch frozen（write 逐 section 多 call,A 路线,re-grill locked `_WRITE_PROMPT`）**：write 从 one-shot（1 call 整篇 `{report}`）→ **逐 section sequential**（overview + per-dimension[brief.dimensions] + conclusion，N+2 call，主 session `harness.prompt` 串行——**保 D24 session JSONL 持久化 + SSE 可见 + v0.2.5 memory_inject 测试不改**；parallel[completeSimple/ephemeral] defer 因丢 session JSONL + SSE + 测试）。每 section call 收 brief + 整 coverage map + dimension 指令（"聚焦该维度 cell"）+ **memory blob（v0.2.5 每 section 注入）**；输出 `{body, sources}`（prompt 指令 JSON + 容错解析，**不强求 response_format**——跟现 write 一致）；best-effort per section（单 section 挂→body="(本节生成失败)" + 继续，不搞挂整篇 write）。`_assemble_report` 拼 `## {title}\n{body}` 顺序 + `## Sources`（按 section 分组 `### {title}\n[n] url`，无全局 [n] 重编号）+ sections[] 顺序 id "1".."N+1"（跟 `_split_sections` 约定一致，body 含 `## title` 行——**refine/ReportPage 不变**）。落点：`profiles._WRITE_PROMPT` → per-section writer system prompt（v0.2.6 re-grill，仍 `tool_names=[]`）；`research_runner._run_write_stage`（新，替换 write 的 one-shot 路径）+ `_section_list`/`_build_section_prompt`/`_extract_section_output`/`_assemble_report`。**诚实边界**：无 audit/rework（B 路线 deferred）、仍 markdown（非结构化组件 `VPricingTable` 等，B deferred）、~N+2 call（3-5，介于 one-shot 1 和 VerdaAI 7-10）。契约 0.3.9 不动（app/workflow 行为变更，无 ADR）；http v0.3.4 不动（无新路由）；ROADMAP 0.1.46；offline 7 测试（assemble unit 5 + integration 2）+ live 真 run（443s 验 report per-section 结构）+ v0.2.5 memory_inject live 回归通过（405s，blob 仍进 section prompt→session JSONL） |
| 0.2.7 | 2026-08-06 | **patch frozen（per-task 搜索超参覆盖,配合 http v0.3.5）**：`CoverageEngine.__init__` 加 `max_queries`/`max_wall_seconds`（per-task Budget 覆盖,POST /tasks `search_overrides` 透传）;run 里初始化 SOCM 后覆盖 `state.budget`;`ResearchRunner.search_overrides` 透传 CoverageEngine(max_parallel/coverage_threshold/max_queries/max_wall_seconds)。无覆盖=env 默认(向后兼容)。契约 0.3.12 不动;http v0.3.4→v0.3.5;ROADMAP 0.1.51 |
| 0.2.8 | 2026-08-06 | **patch frozen（query 竞品兜底,VerdaAI 对齐）**：bug——"为飞书、钉钉、企业微信做 SWOT"类 query,discover 把列举产品当 subject(品类)→ 候选是 Slack/Teams(漏飞书/钉钉/企业微信)→ 跳过竞品错。修:① discover + derive prompt 加"query 显式列的多产品(X、Y、Z / X vs Y)当竞品必进,subject=共属品类";② `_regex_brands(query)`(VerdaAI `_regex_brands` 移植:切顿号/vs/与/和 + 停用词 + 2-8 字 + 上限 6)作 **fallback**(`fallback_brands = user_brands or _regex_brands(query)`,VerdaAI 式——只在用户没勾选时兜底);③ `_coerce_competitors` 加 `must_include`(fallback_brands 硬塞保 query 产品必搜,LLM 不听话兜底)。诚实边界:正则对中文列举(有顿号)准;产品名后接中文描述无分隔切不净(主靠 LLM prompt,正则只兜底);契约 0.3.12 不动;http v0.3.5 不动;ROADMAP 0.1.52;offline 11 测试(regex unit 10 + integration 1)+ 44 回归全绿 |
| 0.2.9 | 2026-08-09 | **patch frozen（子 agent 多轮 + reasonix compaction 上线,ADR 0016）**：根因——pi4 compaction 在线从不工作。实验 C 证明两层:① 子 agent `build_ephemeral` 以 `capability_report=None` 构造,`_bind_extension_context` 在 `__init__` 末尾跑时 `extension_runner` 仍 None → 早返 → `getContextUsage`/`compact` 从未绑;之后 `attach_extension_runtime` → `Agent.set_extension_runner` 绑**默认** context_actions(`getContextUsage=lambda: None`),reasonix `turn_end` 拿 None → `context_usage_unavailable` → 永不压缩。② 单 prompt ReAct → session 仅 1 条 user → reasonix `before_compact` `activeTurnEntryIds=全部` → fold=∅ → 即便触发也不产 entry。修(A1):① 底座 `AgentHarness.attach_extension_runtime_and_rebind(result, cwd, *, replace)`(ADR 0016,pi_agent 0.81.1→0.81.2)——attach + `_bind_extension_context` 重绑,覆盖默认值;② app 层 `wiring.build_ephemeral` 用新方法挂 Extraction + reasonix(同一 fresh runtime,一次 attach + rebind,reasonix 每 ephemeral 现场加载保 `_State` 每 session 独立——**不缓存**,缓存会让并行子 agent 共享 reasonix `_State` 污染 compaction 决策);③ `coverage_engine._run_subagent_prompt` 单 prompt → 2 轮(轮1 搜全 cell → flush intake 进 SOCM → 读空 cell → 轮2 补空 cell,复用 `_build_subagent_prompt`),让 session 有多 user 消息,reasonix 有旧轮可压;全满跳过轮2(单轮);轮1 失败不补轮。落点:`agent_harness.py`(底座)+ `wiring.py`/`coverage_engine.py`(app)+ 新测试。**诚实边界**:真实 1M window 下子 agent fetch 几十网页够不着 ~983k 阈值,大概率仍不触发(机制在线,换短窗口模型自然触发);8k window 是 live 验证手段(非生产);轮2 只补 EMPTY cell(UNKNOWN/CONFLICT terminal 不重搜,跟 `actionable_cells` 一致)。契约 0.3.12 不动(F-R28 文字不变:仍 ephemeral/不落 JSONL/1:1,多 prompt 同一 InMemorySessionRepo);http v0.3.5 不动;ROADMAP 0.1.53;ADR 0016;offline 底座 4 测试 + multiturn 4 测试 + 回归全绿;live 验证(降 contextWindow=8k)确认 compaction 产生 entry |
