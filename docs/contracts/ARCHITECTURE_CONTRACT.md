...
| *(0.3.12 patch)* | 2026-08-09 | **ADR 0016**：pi_agent `AgentHarness.attach_extension_runtime_and_rebind(result, cwd, *, replace)`——迟挂 extension（ephemeral 子 agent）后重绑 context_actions（`getContextUsage`/`compact`），覆盖 `Agent.set_extension_runner` 留下的默认值（`getContextUsage=lambda: None` → reasonix `turn_end` 拿 None 永不压缩，实验 C 证明）。包装既有 `attach_extension_runtime` + `_bind_extension_context`，二者不改。pi_agent 0.81.1→0.81.2（patch，向后兼容）。**不动 D*/G* 核心、不升 contract_version**（未改任何冻结决策；F-R28 文字不变：仍 ephemeral/不落 JSONL/1:1）；reasonix 不缓存（每 ephemeral 现场加载保 `_State` 每 session 独立）；feature `research-workflow-v1` v0.2.8→v0.2.9（子 agent 单 prompt → 2 轮 + 挂 reasonix） |
